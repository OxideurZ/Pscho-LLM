...
async def test_fresh_database_applies_ordered_checksumed_migrations(tmp_path: Path) -> None:
    ...
    assert status.expected_schema_version == 15
    ...
    assert rows == [
        ...
        (8, "jobs", 64),
        (9, "model_run_memory_kinds", 64),
        (10, "memory_core", 64),
        (11, "memory_entities", 64),
        (12, "memory_backfill", 64),
        (13, "retrieval_foundations", 64),
        (14, "retrieval_index_lifecycle", 64),
        (15, "retrieval_audit", 64),
    ]
...
async def test_invalid_or_failed_migration_never_reports_current(tmp_path: Path) -> None:
    ...
    (invalid_order / "017_gap.sql").write_text("SELECT 1;\n", encoding="utf-8")
